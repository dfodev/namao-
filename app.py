...
from forms import LoginForm, RegisterForm, BecomeProviderForm, AvailabilityForm, PortfolioForm
...
def dashboard():
    ...
    if current_user.is_provider:
        ...
        recent_visitors = ProfileView.query.filter_by(user_id=current_user.id) \
            .order_by(ProfileView.viewed_at.desc()).limit(5).all()
        portfolio_images = PortfolioImage.query.filter_by(user_id=current_user.id) \
            .order_by(PortfolioImage.id.desc()).all()
                ...
                if file and file.filename != '':
                    ...
                    try:
                        ...
                        new_image = PortfolioImage(
                            ...
                            title=form.title.data or "Trabalho Realizado",
                            description=form.description.data or ""
                        )
                        db.session.add(new_image)
                    except Exception as e:
                        db.session.rollback()
                        flash(f'Erro ao processar a imagem {filename}: {str(e)}', 'danger')
                        return redirect(url_for('dashboard'))
            db.session.commit()
            flash('Imagens adicionadas ao portfólio com sucesso!', 'success')
    else:
...
def delete_portfolio(image_id):
    ...
    if image.user_id != current_user.id:
        flash('Acesso negado')
        return redirect(url_for('dashboard'))
    file_path = os.path.join('static/uploads', image.image_path)
...
def update_profile():
    try:
        ...
        preco_medio = request.form.get('preco_medio', '').strip()
        if name:
            current_user.name = name[:100]
        if cidade:
            current_user.cidade = cidade[:100]
        if bairro:
            current_user.bairro = bairro[:100]
        if telefone:
        ...
        if current_user.is_provider and preco_medio:
            service = Service.query.filter_by(user_id=current_user.id).first()
            if service:
...
def perfil_publico(user_id):
    ...
    if is_authenticated and current_user.id == user.id:
        reviews = Review.query.filter_by(provider_id=user_id, status='approved').order_by(
            Review.created_at.desc()).all()
        return render_template('perfil_publico.html', user=user, service=service, portfolio=portfolio, reviews=reviews,
                               is_favorite=False)
    tempo_limite = datetime.utcnow() - timedelta(minutes=15)
    query_filtro = ProfileView.query.filter(ProfileView.user_id == user_id, ProfileView.viewed_at >= tempo_limite)
    if is_authenticated:
    ...
    is_favorite = Favorite.query.filter_by(user_id=current_user.id,
                                           provider_id=user_id).first() is not None if is_authenticated else False
    return render_template('perfil_publico.html', user=user, service=service, portfolio=portfolio, reviews=reviews,
...
def favoritar(provider_id):
    if current_user.id == provider_id:
        return jsonify({'status': 'error', 'message': 'Você não pode favoritar o seu próprio perfil.'}), 400
    ...
    try:
        if existing_favorite:
            ...
            db.session.commit()
            return jsonify(
                {'status': 'success', 'action': 'removed', 'message': 'Removido dos favoritos com sucesso.'}), 200
        else:
            new_favorite = Favorite(user_id=current_user.id, provider_id=provider_id)
            db.session.add(new_favorite)
            db.session.commit()
            return jsonify(
                {'status': 'success', 'action': 'added', 'message': 'Adicionado aos favoritos com sucesso!'}), 201
    except Exception:
        db.session.rollback()
...
def registrar_contato(provider_id):
    if current_user.id == provider_id:
        return jsonify({'status': 'error', 'message': 'Auto-contatos não geram registros.'}), 400
    User.query.get_or_404(provider_id)
    new_contact = ContactHistory(client_id=current_user.id, provider_id=provider_id)
    try:
        db.session.add(new_contact)
        db.session.commit()
        return jsonify({'status': 'success', 'message': 'Interação de contato computada com sucesso.'}), 201
    except Exception:
        db.session.rollback()
        return jsonify({'status': 'error', 'message': 'Erro interno.'}), 500
...
def avaliar(provider_id):
    ...
    if existing_review:
        ...
        return redirect(url_for('perfil_publico', user_id=provider_id))
    new_review = Review(reviewer_id=current_user.id, provider_id=provider_id, rating=rating, comment=comment,
                        status='approved')
    try:
...
def denunciar(user_id):
    ...
    if not reason:
        flash('Motivo da denúncia é obrigatório.', 'danger')
        return redirect(request.referrer)

    if current_user.id == user_id:
        flash('Você não pode denunciar a si mesmo.', 'warning')
        return redirect(request.referrer)
    ...
    try:
        ...
        db.session.commit()
        flash('Denúncia enviada com sucesso! Nossa equipe analisará em até 48 horas.', 'success')
    except Exception as e:
        db.session.rollback()
...
def busca_avancada():
    ...
    categorias = ['Todos', 'Encanador', 'Eletricista', 'Pedreiro', 'Pintor', 'Jardineiro', 'Entregador', 'Faxineiro',
                  'Outros']
    return render_template('busca_avancada.html', servicos=servicos, bairro=bairro, cidade=cidade, categoria=categoria,
...
def api_profile(user_id):
    ...
    portfolio = PortfolioImage.query.filter_by(user_id=user_id).order_by(PortfolioImage.is_primary.desc(),
    ...
    if availability:
        availability_dict = {
            'Segunda': availability.segunda, 'Terça': availability.terca,
            'Quarta': availability.quarta, 'Quinta': availability.quinta,
            'Sexta': availability.sexta, 'Sábado': availability.sabado,
            'Domingo': availability.domingo
        }
    portfolio_list = [{
        'id': img.id,
        'image_path': img.image_path,
        'image_url': url_for('static', filename='uploads/' + img.image_path.split('/')[-1]),
        'title': img.title,
...
def api_toggle_availability():
    if not current_user.is_provider:
        return jsonify({'success': False, 'message': 'Apenas prestadores.'}), 403
    service = Service.query.filter_by(user_id=current_user.id).first()
    if not service:
    ...
    try:
        ...
        status = "disponível" if service.is_available else "indisponível"
        return jsonify({
            'success': True, 'is_available': service.is_available,
            'message': f'Você agora está {status} na vitrine!',
            'badge_class': 'bg-success' if service.is_available else 'bg-danger',
            'badge_text': 'Disponível' if service.is_available else 'Indisponível',
            'button_class': 'btn-outline-danger' if service.is_available else 'btn-success',
            'button_icon': 'fa-eye-slash' if service.is_available else 'fa-eye',
            'button_text': 'Ficar Indisponível' if service.is_available else 'Ficar Disponível',
            'status_text': 'Disponível na Vitrine' if service.is_available else 'Indisponível'
        })
    except Exception as e:
        db.session.rollback()
...
def api_update_availability():
    ...
    if day not in ['segunda', 'terca', 'quarta', 'quinta', 'sexta', 'sabado', 'domingo']:
    ...
    if not availability:
        ...
        db.session.add(availability)
    setattr(availability, day, value)
    try:
        db.session.commit()
        return jsonify({'success': True, 'day': day, 'value': value,
                        'message': f'{day.capitalize()} {"ativado" if value else "desativado"}!'})
    except Exception as e:
        db.session.rollback()
...
def admin_dashboard():
    ...
    return render_template('admin_dashboard.html', pending_reports=pending_reports, pending_reviews=pending_reviews,
...
# EXCLUIR SERVIÇO
# ==============================================
@app.route('/excluir-servico', methods=['POST'])
@login_required
def excluir_servico():
    if not current_user.is_provider:
        flash('Apenas prestadores podem usar esta funcionalidade.', 'danger')
        return redirect(url_for('dashboard'))
    try:
        service = Service.query.filter_by(user_id=current_user.id).first()
        if service:
        ...
        portfolio_images = PortfolioImage.query.filter_by(user_id=current_user.id).all()
        for img in portfolio_images:
            file_path = os.path.join('static/uploads', img.image_path)
            if os.path.exists(file_path):
                os.remove(file_path)
            db.session.delete(img)
        current_user.is_provider = False
        current_user.bio = ''
        current_user.trust_seal = False
        db.session.commit()
        flash('Seu serviço foi excluído com sucesso! Você agora é um cliente.', 'success')
    except Exception as e:
        db.session.rollback()
...
@app.route('/admin/bloquear-usuario/<int:user_id>')
@login_required
def bloquear_usuario(user_id):
    ...
    # Resolve todas as denúncias pendentes deste usuário
    Report.query.filter_by(reported_id=user_id, status='pending').update({'status': 'resolved'})

    db.session.commit()
    flash(f'Usuário {user.name} foi BLOQUEADO!', 'success')
    return redirect(url_for('admin_dashboard'))
